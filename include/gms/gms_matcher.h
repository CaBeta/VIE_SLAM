...
public:
	...
	gms_matcher(const std::vector<KeyPoint> &vkp1, const cv::Size size1, const std::vector<KeyPoint> &vkp2, const cv::Size size2, const std::vector<DMatch> &vDMatches)
	{
		// Input initialize
...
private:
	...
	void NormalizePoints(const std::vector<KeyPoint> &kp, const cv::Size &size, std::vector<Point2f> &npts) {
	...
	void ConvertMatches(const std::vector<DMatch> &vDMatches, std::vector<pair<int, int> > &vMatches) {
...
// utility
inline Mat DrawInlier(Mat &src1, Mat &src2, std::vector<KeyPoint> &kpt1, std::vector<KeyPoint> &kpt2, std::vector<DMatch> &inlier, int type) {
